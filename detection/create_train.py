    ...
    for cc in range(2):
        for ct, name in enumerate(files):
            ...
            while 1:
                ...
                img_png=transform.rotate(img_png,rand_angle)
                img_png=(img_png*256).astype(np.uint8)

                change_color_zero=np.array([0,0,0,0])
                change_color=np.array([random.randint(0,255),random.randint(0,255),random.randint(0,255),random.randint(230,255)])

                for i in range(img_png.shape[0]):
                    for j in range(img_png.shape[1]):
                        if sum(img_png[i][j])>710 or img_png[i][j][3]<=20:
                            img_png[i][j]=change_color_zero
                for i in range(img_png.shape[0]):
                    for j in range(img_png.shape[1]):
                        if sum(img_png[i][j])>0:
                            img_png[i][j]=change_color
            ...
            cv2.imwrite(location + '_train/images/' + str(cc) +name.split('/')[-1], res_img)
            with open(location + '_train/label.csv',"a",newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([str(cc) +name.split('/')[-1], label, x1, y1, x2, y2])
